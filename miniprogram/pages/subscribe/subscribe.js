...
function request(method, path, data) {
  return new Promise(function(resolve, reject) {
    wx.request({
      ...
      success: function(r) { if (r.statusCode === 200) resolve(r.data); else reject(r) },
      fail: reject,
    })
  })
}
...
Page({
  data: {
    keywords: [], brands: [], notifyEnabled: true,
    alerts: [], alertCount: 0, keywordList: [], brandList: [],
    _rawKeywords: ['零添加', '有机酱油', '减盐', '生抽', '老抽', '蚝油'],
    _rawBrands: ['海天', '千禾', '李锦记', '厨邦', '加加'],
  },

  onLoad: function() { this.initLists(); this.loadSubscription(); this.checkAlerts() },
  onShow: function() { this.checkAlerts() },

  initLists: function() {
    this.setData({
      keywordList: this.data._rawKeywords.map(function(k) { return { name: k, selected: false } }),
      brandList: this.data._rawBrands.map(function(b) { return { name: b, selected: false } }),
    })
  },

  _refreshLists: function() {
    var keys = this.data.keywords, brs = this.data.brands
    this.setData({
      keywordList: this.data._rawKeywords.map(function(k) { return { name: k, selected: keys.indexOf(k) > -1 } }),
      brandList: this.data._rawBrands.map(function(b) { return { name: b, selected: brs.indexOf(b) > -1 } }),
    })
  },

  async loadSubscription() {
    try {
      var sub = await request('GET', '/api/notify/subscription')
      this.setData({
  ...
  async checkAlerts() {
    try {
      var res = await request('GET', '/api/notify/alerts')
  ...
  toggleKeyword: function(e) {
    ...
    this._refreshLists()
    request('POST', '/api/notify/subscription', {
  ...
  manualCheck: function() {
    ...
    request('GET', '/api/notify/check').then(function() {
      self.checkAlerts(); wx.hideLoading()
